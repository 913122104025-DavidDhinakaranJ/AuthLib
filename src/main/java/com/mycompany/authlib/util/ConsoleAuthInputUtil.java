...
public class ConsoleAuthInputUtil {
    ...
    public String getUsername(boolean isValidate) {
        while (true) {
            System.out.print("Enter username: ");
            String username = scanner.next().trim();

            if(!isValidate || username.matches("^[a-zA-Z0-9]{4,20}$")) return username;
            
            displayError("Username must be 4–20 characters, letters and digits only.");
        }
    }
    
    public String getPassword(String prompt, boolean isValidate) {
        int MIN_LENGTH = 8;
        int MAX_LENGTH = 64;
        String SPECIALS = "@$!%*?&";
        while (true) {
